...
void fof_fof(int num)
{
  ...
  Head = malloc(NumPart * sizeof(int));
  Len = malloc(NumPart * sizeof(int));
  Next = malloc(NumPart * sizeof(int));
  Tail = malloc(NumPart * sizeof(int));
  MinID = malloc(NumPart * sizeof(int));
  MinIDTask = malloc(NumPart * sizeof(int));

  local_toGo = malloc(sizeof(int) * NTask);
  toGo = malloc(sizeof(int) * NTask * NTask);
  export_offset = malloc(sizeof(int) * NTask);
  import_offset = malloc(sizeof(int) * NTask);
...
void fof_exchange_id_lists(void)
{
  ...
  local_ids = malloc(1 + Nlocal * sizeof(struct id_list));
  export_ids = malloc(1 + nsend * sizeof(struct id_list));
  ...
  for(n = 0; n < NumPart; n++)
      ...
      if(task != ThisTask)
	{
	  ...
	  if(P[n].Type == 0)
	    {
	      export_ids[export_offset[task] + local_toGo[task]].Density = SphP[n].Density;
#ifdef SFR
      ...
      else
	{
	  ...
	  if(P[n].Type == 0)
	    {
	      local_ids[local_toGo[task]].Density = SphP[n].Density;
#ifdef SFR
...
#ifdef BLACK_HOLES
  GroupMbh = malloc((1 + Ngroups * sizeof(float)));
  GroupMdot = malloc((1 + Ngroups * sizeof(float)));
#endif
  GroupLen = malloc(1 + Ngroups * sizeof(int));
  GroupLenType = malloc(6 * (1 + Ngroups * sizeof(int)));
  GroupMassType = malloc(6 * (1 + Ngroups * sizeof(double)));
  GroupCM = malloc(3 * (1 + Ngroups * sizeof(float)));
  GroupSfr = malloc((1 + Ngroups * sizeof(float)));
  GroupOffset = malloc(1 + Ngroups * sizeof(int));
  GroupIDs = malloc(1 + Nids * sizeof(int));


#ifdef BUBBLES
  BiggestGroup = malloc(NTask * sizeof(struct biggest_group));
#endif
  ...
  for(i = 0, j = 0, GroupOffset[0] = 0; i < Ngroups; i++)
      ...
      for(k = 0; k < GroupLen[i]; k++)
	{
	  ...
	  if(local_ids[j + k].Type == 5)
	    {
	      GroupMdot[i] += local_ids[j + k].BH_Mass;
	      GroupMbh[i] += local_ids[j + k].BH_Mdot;
	    }
#endif
	}
...
void fof_make_black_holes(void)
{
  ...
  for(i = 0; i < Ngroups; i++)
    {
      ...
      if(GroupLenType[i * 6 + 1] * All.MassTable[1] >=
  ...
  local_indices = malloc(nlocal * sizeof(int));
  export_indices = malloc(nsend * sizeof(int));
...
void fof_find_nearest_dmparticle(void)
{
  ...
  int *noffset, *nbuffer, *nsend_local, *nsend;
  MPI_Status status;
  fof_nearest_distance = malloc(sizeof(float) * NumPart);
  fof_nearest_hsml = malloc(sizeof(float) * NumPart);
  noffset = malloc(sizeof(int) * NTask);	/* offsets of bunches in common list */
  nbuffer = malloc(sizeof(int) * NTask);
  nsend_local = malloc(sizeof(int) * NTask);
  nsend = malloc(sizeof(int) * NTask * NTask);
  ...
  free(nbuffer);
  free(noffset);
  free(fof_nearest_hsml);
  free(fof_nearest_distance);
  if(ThisTask == 0)
    {
...
void find_CM_of_biggest_group(void)
{
  ...
  BGroupLen = malloc(sizeof(int));
  BGroupLen_common = malloc(NTask * sizeof(int));
  BGroupCM = malloc(3 * sizeof(float));
  BGroupCM_common = malloc(3 * NTask * sizeof(float));
  BGroupMass = malloc(6 * sizeof(double));
  BGroupMass_common = malloc(6 * NTask * sizeof(double));
  ...
  free(BGroupLen);
  free(BGroupLen_common);
  free(BGroupCM);
  free(BGroupCM_common);
  free(BGroupMass);
  free(BGroupMass_common);
...
#define boxHalf_Z boxHalf
  ...
  for(k=0; k < Ngroups; k++)
      ...
      if(massDMpart > 0)
	{
	  if(GroupLenType[k * 6 + 1] * massDMpart >=
	     (All.Omega0 - All.OmegaBaryon) / All.Omega0 *
	     All.MinFoFMassForNewSeed)
	    nheat ++;
	}
      else
	{
  ...
  nn_heat = malloc(NTask * sizeof(int));
  disp = malloc(NTask * sizeof(int));
  ...
  if(tot_nheat > 0)
    {
      GroupMassType_common = malloc(tot_nheat * sizeof(double));
      GroupMassType_dum = malloc(nheat * sizeof(double));
      
      GroupCM_common_x = malloc(tot_nheat * sizeof(float));
      GroupCM_dum_x = malloc(nheat * sizeof(float));
      
      GroupCM_common_y = malloc(tot_nheat * sizeof(float));
      GroupCM_dum_y = malloc(nheat * sizeof(float));
      
      GroupCM_common_z = malloc(tot_nheat * sizeof(float));
      GroupCM_dum_z = malloc(nheat * sizeof(float));
      ...
      for(k=0, i=0; k < Ngroups; k++)
	{
      ...
      MPI_Allgatherv(GroupMassType_dum, nheat, MPI_DOUBLE, GroupMassType_common, nn_heat, disp, MPI_DOUBLE, MPI_COMM_WORLD);
      
      MPI_Allgatherv(GroupCM_dum_x, nheat, MPI_FLOAT, GroupCM_common_x, nn_heat, disp, MPI_FLOAT, MPI_COMM_WORLD);
      
      MPI_Allgatherv(GroupCM_dum_y, nheat, MPI_FLOAT, GroupCM_common_y, nn_heat, disp, MPI_FLOAT, MPI_COMM_WORLD);
	      
      MPI_Allgatherv(GroupCM_dum_z, nheat, MPI_FLOAT, GroupCM_common_z, nn_heat, disp, MPI_FLOAT, MPI_COMM_WORLD);
      ...
      for(l=0; l< tot_nheat; l++) 
	{
	  if(All.ComovingIntegrationOn > 0)
	    {
	      BubbleDistance = All.BubbleDistance * 1./All.Time * pow(GroupMassType_common[l] / All.ClusterMass200 , 1./3.) / pow(hubble_a, 2./3.);
	      
	      BubbleRadius = All.BubbleRadius * 1./All.Time *  pow(GroupMassType_common[l] / All.ClusterMass200 , 1./3.) / pow(hubble_a, 2./3.);
	      
	      BubbleEnergy = All.BubbleEnergy * pow(GroupMassType_common[l] / All.ClusterMass200 , 5./3.) * pow(hubble_a, 2./3.);    
	      ...
	      do
		{
		  ...
		  for(n = 0; n < numngb_inbox; n++)
		      ...
		      if(r2 < BubbleRadius * BubbleRadius)
			{
			  ...
			  if(All.ComovingIntegrationOn)
			    E_bubble += SphP[j].Entropy * P[j].Mass * pow(SphP[j].Density / pow(All.Time, 3), GAMMA_MINUS1) / GAMMA_MINUS1;
			  else
			    E_bubble += SphP[j].Entropy * P[j].Mass * pow(SphP[j].Density, GAMMA_MINUS1) / GAMMA_MINUS1;
	      ...
	      if(ThisTask==0)
		{
		  if(logical == 1)
		    {
		      printf("%g, %g, %g, %g, %d, %g, %g, %g\n", GroupMassType_common[l],GroupCM_common_x[l],GroupCM_common_y[l],GroupCM_common_z[l],tot_numngb,BubbleRadius,BubbleEnergy,(BubbleEnergy + totE_bubble)/totE_bubble);
		...
		for(n = 0; n < numngb_inbox; n++)
		  {
			...
			if(All.ComovingIntegrationOn)
			  SphP[j].Entropy += GAMMA_MINUS1 * dE / P[j].Mass / pow(SphP[j].Density / pow(All.Time, 3), GAMMA_MINUS1);
			else
			  SphP[j].Entropy += GAMMA_MINUS1 * dE / P[j].Mass / pow(SphP[j].Density, GAMMA_MINUS1); 
			if(dE > 0)
			  P[j].ID = -P[j].ID;
    ...
    if(tot_nheat > 0)
      ...
      free(GroupCM_common_x);
      free(GroupCM_dum_x);
      
      free(GroupCM_common_y);
      free(GroupCM_dum_y);
      
      free(GroupCM_common_z);
      free(GroupCM_dum_z);
      
      free(GroupMassType_common); 
      free(GroupMassType_dum);
