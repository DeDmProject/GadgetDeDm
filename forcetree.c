...
/*! \file forcetree.c
 *  \brief gravitational tree(s) and code for Ewald correction
 *
 *  This file contains the computation of the gravitational force by
 *  means of a tree. The type of tree implemented is a geometrical
 *  oct-tree, starting from a cube encompassing all particles. This
 *  cube is automatically found in the domain decomposition. Tree
 *  nodes can be dynamically updated in drift/kick operations to avoid
 *  having to reconstruct the tree every timestep.  
*/
...
/*! Constructs the gravitational oct-tree(s).  For particle types with
 *  identical softening a common tree is constructed, otherwise a separate
 *  tree.  We first construct a "pretree", with the multipole moments computed
 *  recursively.  This allows the final tree nodes to be ordered such that a
 *  full tree walk traverses the nodes sequentially in physical memory.
 *
...
{
  Numnodestree = force_treebuild_single();
...
/*! This function constructs a common tree for the particle types flagged in
 *  the 'typelist' array.
 */
int force_treebuild_single(void)
{
  ...
  for(i = 0; i < NTopleaves; i++)
    {
      ...
      DomainMoment[i].s[2] = Nodes[index].center[2];
	   ...
	   /* 
	    * Since Nodes[i].center[j] is the geometrical center of the node it will be the same for all particles
            * Hence, no need to add a center_dm variable 
	    */
#ifdef DEDM_TREE
      DomainMoment[i].mass_dm = 0;
      DomainMoment[i].s_dm[0] = Nodes[index].center[0];
      DomainMoment[i].s_dm[1] = Nodes[index].center[1];
      DomainMoment[i].s_dm[2] = Nodes[index].center[2];
#endif
 ...
 *  and all its subnodes using a recursive computation.  The result is stored
 *  in the Nodes[] structure in the sequence of this tree-walk.  Note that
 *  therefore Prenodes[] and Nodes[] are not stored in the same order, but the
 *  connection can be made using the 'nodeindex' variable. 
 *  
 *  Note that the bitflags-variable for each node is used to store in the lowest
 *  bits some special information: Bit 0 flags whether the node belongs to the
 *  top-level tree corresponding to the domain decomposition, while Bit 1
 *  signals whether the top-level node is dependent on local mass.
 * 
...
#ifdef DEDM_TREE 
	mass_dm += Nodes[p].u.d.mass_dm;
	s_dm[0] += Nodes[p].u.d.mass_dm * Nodes[p].u.d.s_dm[0];
	s_dm[1] += Nodes[p].u.d.mass_dm * Nodes[p].u.d.s_dm[1];
	s_dm[2] += Nodes[p].u.d.mass_dm * Nodes[p].u.d.s_dm[2];
	vs_dm[0] += Nodes[p].u.d.mass_dm * Extnodes[p].vs_dm[0];
	vs_dm[1] += Nodes[p].u.d.mass_dm * Extnodes[p].vs_dm[1];
	vs_dm[2] += Nodes[p].u.d.mass_dm * Extnodes[p].vs_dm[2];
#endif
...
{		// TODO: add other particle types
		...
		else 
	{
      ...
      Nodes[no].u.d.s_dm[0] = s_dm[0];
      Nodes[no].u.d.s_dm[1] = s_dm[1];
      Nodes[no].u.d.s_dm[2] = s_dm[2];
      Nodes[no].u.d.mass_dm = mass_dm;
#endif
...
/*! This function updates the multipole moments of the pseudo-particles that
 *  represent the mass distribution on different CPUs. For that purpose, it
 *  first exchanges the necessary data, and then updates the top-level tree
 *  accordingly. The detailed implementation of these two tasks is done in
 *  separate functions.
 */
  ...
  for(i = DomainMyStart; i <= DomainMyLast; i++)
    {
...
#ifdef DEDM_TREE
      DomainMoment[i].s_dm[0] = Nodes[no].u.d.s_dm[0];
      DomainMoment[i].s_dm[1] = Nodes[no].u.d.s_dm[1];
      DomainMoment[i].s_dm[2] = Nodes[no].u.d.s_dm[2];
      ...
      DomainMoment[i].mass_dm = Nodes[no].u.d.mass_dm;
#endif
...
#ifdef DEDM_TREE
  FLOAT sold_dm[3], vsold_dm[3], snew_dm[3], vsnew_dm[3], massold_dm, massnew_dm, mm_dm;
#endif
...
#ifdef DEDM_TREE
	    sold_dm[k] = Nodes[no].u.d.s_dm[k];
	    vsold_dm[k] = Extnodes[no].vs_dm[k];
#endif
...
#ifdef DEDM_TREE
massold_dm = Nodes[no].u.d.mass_dm;
#endif
...
#ifdef DEDM_TREE
	snew_dm[k] = DomainMoment[i].s_dm[k];
	vsnew_dm[k] = DomainMoment[i].vs_dm[k];
#endif
	  }
	massnew = DomainMoment[i].mass;

#ifdef DEDM_TREE
	massnew_dm = DomainMoment[i].mass_dm;
#endif
...
#ifdef DEDM_TREE
	mm_dm = Nodes[no].u.d.mass_dm + massnew_dm - massold_dm;
#endif
	    for(k = 0; k < 3; k++)
	      {
		if(mm > 0)
		  {
		    ...
		    if(mm_dm >0){
			    Nodes[no].u.d.s_dm[k] = 
	(Nodes[no].u.d.mass_dm*Nodes[no].u.d.s_dm[k] + massnew_dm*snew_dm[k] - massold_dm*sold_dm[k]) / mm_dm;
			    Extnodes[no].vs_dm[k] = 
	(Nodes[no].u.d.mass_dm * Extnodes[no].vs_dm[k] + massnew_dm*vsnew_dm[k] - massold_dm*vsold_dm[k]) / mm_dm;
		    }
#endif
		  }
	      }
...
/*! This function flags nodes that is a driver routine of
 *  force_flag_localnodes(), which is used to flag tree nodes in the top-level
 *  tree that are dependent on local particle data.
 */
 ...
 *  enlarged because particles moved out of their original bound.
 */
...
/*! This function recursively enlarges nodes such that they always contain all
 *  its daughter nodes and daughter particles.
 */
...
/*! This function recursively enlarges nodes of the top-level tree such that
 *  they always contain all its daughter nodes.
 */
...
/*! This routine computes the gravitational force for a given local particle,
 *  or for a particle in the communication buffer.  Each separate tree is
 *  walked, summing the result. Depending on the value of
 *  TypeOfOpeningCriterion, either the geometrical BH cell-opening criterion,
 *  or the `relative' opening criterion is used.
 */

// TODO: not modified for DEDM since it's not planned to run Tree-only simulations
...
#ifdef DEDM_TREE
  double acc_x_dm, acc_y_dm, acc_z_dm, pos_x_dm, pos_y_dm, pos_z_dm, aold_dm;
  double dx_dm, dy_dm, dz_dm, r_dm, r2_dm;
  double mass_dm, fac_dm, u_dm;
  int ptype_dm, tabindex_dm, ninteractions_dm;
#endif

#ifdef EVALPOTENTIAL
  double wp, facpot, pot = 0;
#endif
...
#ifdef DEDM_TREE
if(P[target].Type == 1) // TODO: add other particle types
{
  pos_x_dm = P[target].Pos[0];
  pos_y_dm = P[target].Pos[1];
  pos_z_dm = P[target].Pos[2];
 // ptype_dm = 1; 
} 
#endif
...
#endif // AOLD used only in the BH node opening criterion, we don't need another for dedm interactions only 
      aold = All.ErrTolForceAcc * GravDataGet[target].w.OldAcc;

#ifdef DEDM_TREE
      if(GravDataGet[target].v.Type == 1)
      { // TODO: Add other particle types
	pos_x_dm = GravDataGet[target].u_dm.Pos_dm[0];
	pos_y_dm = GravDataGet[target].u_dm.Pos_dm[1];
	pos_z_dm = GravDataGet[target].u_dm.Pos_dm[2];
  ...
  while(no >= 0)
      ...
      if(no < All.MaxPart)
	{
	  ...
	  if(P[no].Type == 1) 
	{
	  dx_dm = P[no].Pos[0] - pos_x;
	  dy_dm = P[no].Pos[1] - pos_y;
	  dz_dm = P[no].Pos[2] - pos_z;
	...
	dz_dm = NEAREST(dz_dm);
#endif
	r2_dm = dx_dm*dx_dm + dy_dm*dy_dm + dz_dm*dz_dm;
	mass_dm = P[no].Mass;
...
#ifdef DEDM_TREE
	//if(Nodes[no].Type == 1) // || GravDataGet[target].v.Type == 1)
	  // In each node there will always be DM particles (?) 

  mass_dm = nop->u.d.mass_dm;

	if(mass_dm>0)  {
  dx_dm = nop->u.d.s_dm[0] - pos_x_dm;
  dy_dm = nop->u.d.s_dm[1] - pos_y_dm;
  dz_dm = nop->u.d.s_dm[2] - pos_z_dm;
	  ...
	  if(r2 > rcut2)
	    {
	      ...
	      // We don't do any check for dedm interactions, since they are smaller than gravity anyways
...
if(r >= h)
	...
	{ 
	  ...
	  facpot = mass * h_inv * wp;
		...
		/* calculate short-range correction for DM particles */
#ifdef DEDM_TREE
if(r_dm !=0){
if(r_dm >= h)  
{
	fac_dm = mass_dm / (r2_dm * r_dm);
	} 
else 
{
 	u_dm = r_dm * h_inv;
	  if(u_dm < 0.5)
	    fac_dm = mass_dm * h3_inv * (10.666666666667 + u_dm * u_dm * (32.0 * u_dm - 38.4));
	  else
	    fac_dm =
	      mass_dm * h3_inv * (21.333333333333 - 48.0 * u_dm +
	  38.4 * u_dm * u_dm - 10.666666666667 * u_dm * u_dm * u_dm - 0.066666666667 / (u_dm * u_dm * u_dm));
}
	tabindex_dm = (int) (asmthfac * r_dm); //TODO asmthfac_dm ???
	  if(tabindex_dm < NTAB) {
	  fac_dm *= shortrange_table[tabindex_dm];
	acc_x_dm += dx_dm * fac_dm;
	acc_y_dm += dy_dm * fac_dm;
	acc_z_dm += dz_dm * fac_dm;
	  }
...
#ifdef DEDM_TREE
	P[target].DeDmAccel[0] = acc_x_dm;
	P[target].DeDmAccel[1] = acc_y_dm;
	P[target].DeDmAccel[2] = acc_z_dm;
#endif
...
#ifdef DEDM_TREE
	GravDataResult[target].u_dm.Acc_dm[0] = acc_x_dm;
	GravDataResult[target].u_dm.Acc_dm[1] = acc_y_dm;
	GravDataResult[target].u_dm.Acc_dm[2] = acc_z_dm;
#endif
 ...
 *  tree(s). The same opening criteria is used as for the gravitational force
 *  walk.
 */
