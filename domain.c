...
int domain_findSplit(int cpustart, int ncpu, int first, int last)
{
  ...
  if(((double) sphload_leftOfSplit) / ncpu_leftOfSplit > maxloadsph ||
     ((double) (sphload - sphload_leftOfSplit)) / (ncpu - ncpu_leftOfSplit) > maxloadsph)
    {
      /* we did not find a viable split */
      return -1;
    }
 ...
 *  domain_findSplit() with respect to wrok-load balance.  To this end, the 
