  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(P[i].Type == 0)
	{
...
#endif
	  SphP[i].Density *= exp(-SphP[i].u.s.DivVel * dt_drift);
	  PPP[i].Hsml *= exp(0.333333333333 * SphP[i].u.s.DivVel * dt_drift);
	  ...
	  if(SphP[i].Density * a3inv >= All.PhysDensThresh)
	    SphP[i].Pressure =
	      All.FactorForSofterEQS * (SphP[i].Entropy + SphP[i].DtEntropy * dt_entr) * pow(SphP[i].Density,
											     GAMMA) + (1 -
												       All.
												       FactorForSofterEQS)
	      * GAMMA_MINUS1 * SphP[i].Density * All.InitGasU;
	  else
	    SphP[i].Pressure = (SphP[i].Entropy + SphP[i].DtEntropy * dt_entr) * pow(SphP[i].Density, GAMMA);
#endif
#else
