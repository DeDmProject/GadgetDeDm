...
#ifdef DEDM_INFO
	if(ThisTask==0) 
		fprintf(stdout, "Task=%d, Calculating new particle's masses, modified by factor%lf.\n", 
				ThisTask, delta_m);
		fprintf(All.outDeDmFile, "Task=%d, Calculating new particle's masses, modified by factor %lf.\n", 
				ThisTask, delta_m);
#endif
     	for(n=0; n<NumPart; n++)
			if(P[n].Type==1) 
