...
#ifdef DEDM_PM
static fftw_real *rhogrid2, *forcegrid2, *workspace2;
static fftw_complex *fft_of_rhogrid2;
#endif
...
void pm_init_periodic_allocate(int dimprod)
{
  ...
  /* DEDM_PM: the memory to be stored is doubled due to the 
   * need of storing fftw stuff for the new field interactions
   * */

#ifdef DEDM_PM
  if(!(rhogrid2 = (fftw_real *) malloc(bytes = fftsize * sizeof(fftw_real))))
    {
      printf("failed to allocate memory for `FFT-rhogrid2' (%g MB).\n", bytes / (1024.0 * 1024.0));
      endrun(1);
    }
  bytes_tot += bytes;

  if(!(forcegrid2 = (fftw_real *) malloc(bytes = IMAX(fftsize, dimprodmax) * sizeof(fftw_real))))
    {
      printf("failed to allocate memory for `FFT-forcegrid2' (%g MB).\n", bytes / (1024.0 * 1024.0));
      endrun(1);
    }
  bytes_tot += bytes;

  if(!(workspace2 = (fftw_real *) malloc(bytes = IMAX(maxfftsize, dimprodmax) * sizeof(fftw_real))))
    {
      printf("failed to allocate memory for `FFT-workspace2' (%g MB).\n", bytes / (1024.0 * 1024.0));
      endrun(1);
    }
  bytes_tot += bytes;
...
#ifdef DEDM_PM
  fft_of_rhogrid2 = (fftw_complex *) & rhogrid2[0];
#endif 
}
...
#ifdef DEDM_PMb
void pm_dedm_force_periodic(void)
{
  ...
  fac = GTable.G_tilde / (M_PI * All.BoxSize);	/* to get potential */
  ...
  {  
	...
	/*
	 * CIC method, assigning densities to the mesh
	 * We compute density for all particles 
...
if(P[i].Type==1) {
	...
	/* Assign the density to the mesh proportionally to the volume
	 * of the cube overlapping the 8 nearest mesh cells
	 */
  ...
  /* Now calculate the density on the grid for dm particles interacting
   * through modified newton's constant
   * */
}
...
if(P[i].Type==1) {
  ...
  /* add the dark force to the total force felt by dm particles */
	  P[i].DeDmPM[dim] = acc_dim;
	}
    }
}
...
void pmforce_periodic(void)
{
...
#ifdef DEDM_PM
  /* implementing quintessential interaction constant */
  if(ThisTask==0) 
	fprintf(stdout, "Using modified PM algorithm to compute long range dark energy-dark matter interaction.\n");

  double acc_dim2, fac2, asmth22, smth2;
  int ip2;

  fac2 = GTable.G_tilde / (M_PI * All.BoxSize);
  fac2 *= 1 / (2* All.BoxSize / PMGRID);

  asmth22 = (2 * M_PI) * All.Asmth[0] / All.BoxSize;
  asmth22 *= 1 / (2* All.BoxSize / PMGRID);
#endif
...
#ifdef DEDM_PM
      if(P[i].Type == 1){ 
      workspace2[(slab_x * dimy + slab_y) * dimz + slab_z] += P[i].Mass * (1.0 - dx) * (1.0 - dy) * (1.0 - dz);
      workspace2[(slab_x * dimy + slab_yy) * dimz + slab_z] += P[i].Mass * (1.0 - dx) * dy * (1.0 - dz);
      workspace2[(slab_x * dimy + slab_y) * dimz + slab_zz] += P[i].Mass * (1.0 - dx) * (1.0 - dy) * dz;
      workspace2[(slab_x * dimy + slab_yy) * dimz + slab_zz] += P[i].Mass * (1.0 - dx) * dy * dz;

      workspace2[(slab_xx * dimy + slab_y) * dimz + slab_z] += P[i].Mass * (dx) * (1.0 - dy) * (1.0 - dz);
      workspace2[(slab_xx * dimy + slab_yy) * dimz + slab_z] += P[i].Mass * (dx) * dy * (1.0 - dz);
      workspace2[(slab_xx * dimy + slab_y) * dimz + slab_zz] += P[i].Mass * (dx) * (1.0 - dy) * dz;
      workspace2[(slab_xx * dimy + slab_yy) * dimz + slab_zz] += P[i].Mass * (dx) * dy * dz;
	}
#endif
      ...
      if(recvTask < NTask)
	{
	  ...
	  if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
	      ...
	      if(level > 0)
		{
		  ...
		  MPI_Sendrecv(workspace2 + (sendmin - meshmin[0]) * dimy * dimz,
			       (sendmax - sendmin + 1) * dimy * dimz * sizeof(fftw_real), MPI_BYTE, recvTask,
			       TAG_PERIODIC_A, forcegrid2,
			       (recvmax - recvmin + 1) * recv_dimy * recv_dimz * sizeof(fftw_real), MPI_BYTE,
			       recvTask, TAG_PERIODIC_A, MPI_COMM_WORLD, &status);
#endif
		}
	      else
		{
		  ...
		  memcpy(forcegrid2, workspace2 + (sendmin - meshmin[0]) * dimy * dimz,
			 (sendmax - sendmin + 1) * dimy * dimz * sizeof(fftw_real));
#endif
		}
			  ...
			  slab_y <= meshmax_list[3 * recvTask + 1] + 1; slab_y++)
			{
			  ...
			  for(slab_z = meshmin_list[3 * recvTask + 2];
				...
				forcegrid[((slab_x - recvmin) * recv_dimy +
					  ...
					  (slab_z - meshmin_list[3 * recvTask + 2])];
#ifdef DEDM_PM
			      rhogrid2[PMGRID * PMGRID2 * slab_xx + PMGRID2 * slab_yy + slab_zz] +=
				forcegrid2[((slab_x - recvmin) * recv_dimy +
					   (slab_y - meshmin_list[3 * recvTask + 1])) * recv_dimz +
					  (slab_z - meshmin_list[3 * recvTask + 2])];
#endif
			    }
  ...
  rfftwnd_mpi(fft_forward_plan, 1, rhogrid, workspace, FFTW_TRANSPOSED_ORDER);
#ifdef DEDM_PM
  rfftwnd_mpi(fft_forward_plan, 1, rhogrid2, workspace2, FFTW_TRANSPOSED_ORDER);
#endif
  ...
  for(y = slabstart_y; y < slabstart_y + nslab_y; y++)
	...
	smth2 = -exp(-k2 * asmth22) / k2;
	      ...
	      if(kz != 0)
		{
	      ...
	      fft_of_rhogrid[ip].im *= smth;
#ifdef DEDM_PM
	ip2 = PMGRID * (PMGRID / 2+1)*(y - slabstart_y) + (PMGRID / 2 + 1)*x + z;
	fft_of_rhogrid2[ip2].re *= smth2;
	fft_of_rhogrid2[ip2].im *= smth2;
#endif
	    }
	}
  ...
  if(slabstart_y == 0)
    fft_of_rhogrid[0].re = fft_of_rhogrid[0].im = 0.0;
#ifdef DEDM_PM
    fft_of_rhogrid2[0].re = fft_of_rhogrid2[0].im = 0.0; 
#endif
...
#ifdef DEDM_PM
  rfftwnd_mpi(fft_inverse_plan, 1, rhogrid2, workspace2, FFTW_TRANSPOSED_ORDER);
#endif 
			  ...
			  for(slab_y = meshmin_list[3 * recvTask + 1] - 2;
				  ...
				  slab_z <= meshmax_list[3 * recvTask + 2] + 4; slab_z++)
				{
				  ...
				  forcegrid[((slab_x - sendmin) * recv_dimy +
				    ...
				    rhogrid[PMGRID * PMGRID2 * slab_xx + PMGRID2 * slab_yy + slab_zz];
#ifdef DEDM_PM 
				  forcegrid2[((slab_x - sendmin) * recv_dimy +
					     (slab_y - (meshmin_list[3 * recvTask + 1] - 2))) * recv_dimz +
					    slab_z - (meshmin_list[3 * recvTask + 2] - 2)] =
				    rhogrid2[PMGRID * PMGRID2 * slab_xx + PMGRID2 * slab_yy + slab_zz];
#endif
		  ...
		  if(level > 0)
		    {
		      ...
		      MPI_Sendrecv(forcegrid2,
				   (sendmax - sendmin + 1) * recv_dimy * recv_dimz * sizeof(fftw_real),
				   MPI_BYTE, recvTask, TAG_PERIODIC_B,
				   workspace2 + (recvmin - (meshmin[0] - 2)) * dimy * dimz,
				   (recvmax - recvmin + 1) * dimy * dimz * sizeof(fftw_real), MPI_BYTE,
				   recvTask, TAG_PERIODIC_B, MPI_COMM_WORLD, &status);
#endif
		    }
		  else
		    {
		      ...
		      memcpy(workspace + (recvmin - (meshmin[0] - 2)) * dimy * dimz,
			     forcegrid, (recvmax - recvmin + 1) * dimy * dimz * sizeof(fftw_real));
#ifdef DEDM_PM
		      memcpy(workspace2 + (recvmin - (meshmin[0] - 2)) * dimy * dimz,
			     forcegrid2, (recvmax - recvmin + 1) * dimy * dimz * sizeof(fftw_real));
#endif 
		    }
...
#ifdef EVALPOTENTIAL
// TODO: add potential evaluation for DEDM_PM (?)
  for(x = 0; x < meshmax[0] - meshmin[0] + 2; x++)
    for(y = 0; y < meshmax[1] - meshmin[1] + 2; y++)
      for(z = 0; z < meshmax[2] - meshmin[2] + 2; z++)
	{
  ...
  for(i = 0; i < NumPart; i++)	/* read out the potential */
	...
	slab_z = PMGRID - 1;
      ...
      P[i].PM_Potential += pot * (2 * All.BoxSize / PMGRID);	/* compensate the finite differencing factor */
    }
#endif /* evalpotential */
	...
	for(y = 0; y < meshmax[1] - meshmin[1] + 2; y++)
	  for(z = 0; z < meshmax[2] - meshmin[2] + 2; z++)
		...
		fac * ((4.0 / 3) *
	      ...
	      forcegrid2[(x * dimy + y) * dimz + z]
		= /* fac2 now contains the new interaction constant*/
		fac2 * ((4.0 / 3) *
		       (workspace2[((xl + 2) * recv_dimy + (yl + 2)) * recv_dimz + (zl + 2)]
			- workspace2[((xr + 2) * recv_dimy + (yr + 2)) * recv_dimz + (zr + 2)]) -
		       (1.0 / 6) *
		       (workspace2[((xll + 2) * recv_dimy + (yll + 2)) * recv_dimz + (zll + 2)] -
			workspace2[((xrr + 2) * recv_dimy + (yrr + 2)) * recv_dimz + (zrr + 2)]));
...
#ifdef DEDM_PM
	  acc_dim2 =
	    forcegrid2[(slab_x * dimy + slab_y) * dimz + slab_z] * (1.0 - dx) * (1.0 - dy) * (1.0 - dz);
	  acc_dim2 += forcegrid2[(slab_x * dimy + slab_yy) * dimz + slab_z] * (1.0 - dx) * dy * (1.0 - dz);
	  acc_dim2 += forcegrid2[(slab_x * dimy + slab_y) * dimz + slab_zz] * (1.0 - dx) * (1.0 - dy) * dz;
	  acc_dim2 += forcegrid2[(slab_x * dimy + slab_yy) * dimz + slab_zz] * (1.0 - dx) * dy * dz;
	 
	  acc_dim2 += forcegrid2[(slab_xx * dimy + slab_y) * dimz + slab_z] * (dx) * (1.0 - dy) * (1.0 - dz);
	  acc_dim2 += forcegrid2[(slab_xx * dimy + slab_yy) * dimz + slab_z] * (dx) * dy * (1.0 - dz);
	  acc_dim2 += forcegrid2[(slab_xx * dimy + slab_y) * dimz + slab_zz] * (dx) * (1.0 - dy) * dz;
	  acc_dim2 += forcegrid2[(slab_xx * dimy + slab_yy) * dimz + slab_zz] * (dx) * dy * dz;
	  /* add the dark force to the total force felt by dm particles */
	//	if(P[i].Type == 1) // TODO add other Particle Types
	  P[i].DeDmPM[dim] = acc_dim2;
#endif
	}
    }
...
{
	// TODO (?) implement for G' coupled (?) 
	// this function isn't used through the program anyways...
  ...
  if(ThisTask == 0)
    {
      printf("Starting periodic PM potential calculation.\n");
      fflush(stdout);
    }
