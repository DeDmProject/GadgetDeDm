...
void fill_write_buffer(enum iofields blocknr, int *startindex, int pc, int type)
{
  ...
  switch (blocknr)
	...
	if(P[pindex].Type == type)
	  {
	    ...
	    AbundanceRatios(DMAX(All.MinEgySpec,
				 SphP[pindex].Entropy / GAMMA_MINUS1 * pow(SphP[pindex].Density *
									   a3inv,
									   GAMMA_MINUS1)),
			    SphP[pindex].Density * a3inv, &ne, &nh0);
	...
	if(P[pindex].Type == type)
	  {
	    ...
	    tcool = GetCoolingTime(u, SphP[pindex].Density * a3inv, &ne);
	...
	if(P[pindex].Type == type)
	  {
	    *fp++ = SphP[pindex].DissipatedEnergy;
	    n++;
	  }
#endif
      break;
...
int blockpresent(enum iofields blocknr)
{
  ...
  if(All.CoolingOn == 0)
    {
      if(blocknr == IO_NE || blocknr == IO_NH)
	return 0;
    }

  if(All.StarformationOn == 0)
    {
      if(blocknr == IO_SFR || blocknr == IO_AGE || blocknr == IO_Z)
	return 0;
    }
...
#ifndef OUTPUTCOOLRATE
  if(blocknr == IO_COOLRATE)
    return 0;
#endif

#ifndef OUTPUTCONDRATE
  if(blocknr == IO_CONDRATE)
    return 0;
#endif

#ifndef OUTPUTBSMOOTH
  if(blocknr == IO_BSMTH)
    return 0;
#endif

#ifndef OUTPUTDENSNORM
  if(blocknr == IO_DENN)
    return 0;
#endif

#ifndef SFR_PROMOTION
  if(blocknr == IO_EGYPROM)
    return 0;
  if(blocknr == IO_EGYCOLD)
    return 0;
#endif
...
#ifndef COSMIC_RAYS
  if((blocknr == IO_CR_C0) || (blocknr == IO_CR_Q0))
    return 0;
#endif

#ifndef BLACK_HOLES
  if((blocknr == IO_BHMASS) || (blocknr == IO_BHMDOT))
    return 0;
#endif
...
void fill_Tab_IO_Labels(void)
{
  ...
  for(i = 0; i < IO_NBLOCKS; i++)
...
#ifdef CHEMISTRY
      case IO_ELECT:
	strncpy(Tab_IO_Labels[IO_ELECT], "elect ", 4);
	break;
      case IO_HI:
	strncpy(Tab_IO_Labels[IO_HI], "HI ", 4);
	break;
      case IO_HII:
	strncpy(Tab_IO_Labels[IO_HII], "HII ", 4);
	break;
      case IO_HeI:
	strncpy(Tab_IO_Labels[IO_HeI], "HeI ", 4);
	break;
      case IO_HeII:
	strncpy(Tab_IO_Labels[IO_HeII], "HeII ", 4);
	break;
      case IO_HeIII:
	strncpy(Tab_IO_Labels[IO_HeIII], "HeIII ", 4);
	break;
      case IO_H2I:
	strncpy(Tab_IO_Labels[IO_H2I], "H2I ", 4);
	break;
      case IO_H2II:
	strncpy(Tab_IO_Labels[IO_H2II], "H2II ", 4);
	break;
      case IO_HM:
	strncpy(Tab_IO_Labels[IO_HM], "HM ", 4);
	break;
#endif
      case IO_HSML:
	...
	break;
      case IO_AGE:
	strncpy(Tab_IO_Labels[IO_AGE], "AGE ", 4);
	break;
      case IO_Z:
	...
	break;
      case IO_BFLD:
	strncpy(Tab_IO_Labels[IO_BFLD], "BFLD", 4);
	break;
      case IO_DBDT:
	strncpy(Tab_IO_Labels[IO_DBDT], "DBDT", 4);
	break;
      case IO_DIVB:
	strncpy(Tab_IO_Labels[IO_DIVB], "DIVB", 4);
	break;
      case IO_ABVC:
	strncpy(Tab_IO_Labels[IO_ABVC], "ABVC", 4);
	break;
      case IO_COOLRATE:
	strncpy(Tab_IO_Labels[IO_COOLRATE], "COOR", 4);
	break;
      case IO_CONDRATE:
	strncpy(Tab_IO_Labels[IO_CONDRATE], "CONR", 4);
	break;
      case IO_BSMTH:
	strncpy(Tab_IO_Labels[IO_BSMTH], "BFSM", 4);
	break;
      case IO_DENN:
	strncpy(Tab_IO_Labels[IO_DENN], "DENN", 4);
	break;
      case IO_EGYPROM:
	strncpy(Tab_IO_Labels[IO_EGYPROM], "EGYP", 4);
	break;
      case IO_EGYCOLD:
	strncpy(Tab_IO_Labels[IO_EGYCOLD], "EGYC", 4);
	break;
      case IO_CR_C0:
	strncpy(Tab_IO_Labels[IO_CR_C0], "CRC0", 4);
	break;
      case IO_CR_Q0:
	strncpy(Tab_IO_Labels[IO_CR_Q0], "CRQ0", 4);
	break;
      case IO_BHMASS:
	strncpy(Tab_IO_Labels[IO_BHMASS], "BHMA", 4);
	break;
      case IO_BHMDOT:
	strncpy(Tab_IO_Labels[IO_BHMDOT], "BHMD", 4);
	break;
      case IO_MACH:
	strncpy(Tab_IO_Labels[IO_MACH], "MACH", 4);
	break;
      case IO_DISSENERGY:
	strncpy(Tab_IO_Labels[IO_DISSENERGY], "DISSE", 4);
	break;
      }
}
...
#ifdef DEDM_MASS
    if(n == 1) {
		header.mass[n] = All.MassTable[n]*get_variable_mass_factor(All.Time);
#ifdef DEDM_INFO
    if(ThisTask==0)
   {
     fprintf(stdout, "Task=%d, Storing modified DM particles' mass in header file: %e\n", ThisTask, header.mass[n]);
     fprintf(All.outDeDmFile, "Task=%d, Storing modified DM particles' mass in header file: %e\n", 
	ThisTask, header.mass[n]);
   }
#endif
	} else {
